...
#[derive(Clone, Copy)]
pub enum OpCode {IMov, IAdd, ISub, IMul, IXor, ICmp, ITest, ICMove, ICMovg, ICMovge, ICMovl, ICMovle, ILsh, IRsh}

#[derive(Clone, Copy)]
pub enum BranchCode {Jne, Je, Jmp, Jo}
...
pub const TYPE_ERROR: [Instr; 3] = [
    Instr::TwoArg(OpCode::IMov, Loc::Reg(Rq::RSP), Val::Place(Loc::Reg(Rq::RBP))),
...
pub const OVERFLOW_ERROR: [Instr; 3] = [
    Instr::TwoArg(OpCode::IMov, Loc::Reg(Rq::RSP), Val::Place(Loc::Reg(Rq::RBP))),
    Instr::TwoArg(OpCode::IMov, Loc::Reg(Rq::RAX), Val::Imm(OVERFLOW_ERROR_CODE)),
    Instr::Ret
];
    ...
    fn to_string(self: &Self) -> String {
        match self {
            Instr::TwoArg(op, loc, val) => match (loc, val) {
                (Loc::Offset(_, _), _) => format!("{} QWORD {}, {}", op.to_string(), loc.to_string(), val.to_string()),
                (_, Val::Place(Loc::Offset(_, _))) => format!("{} {}, QWORD {}", op.to_string(), loc.to_string(), val.to_string()),
                _ => format!("{} {}, {}", op.to_string(), loc.to_string(), val.to_string()),
            }
            ...
            Instr::CallPrint(reg) => format!("mov {}, QWORD snek_print\ncall {}", as_string(reg), as_string(reg))
        }
    }
}

pub fn instrs_to_str(is: &Vec<Instr>) -> String {
    is.iter().map(|i| i.to_string()).collect::<Vec<String>>().join("\n")
}
// END instrs
