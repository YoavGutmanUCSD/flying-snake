    ...
    match s {
        ...
        Sexp::List(vec) => match &vec[..] {
            [Sexp::Atom(S(op)), e] if op == "add1" => {
                Ok(Expr::UnOp(Add1, Box::new(parse_expr(e)?)))
            }
            [Sexp::Atom(S(op)), e] if op == "sub1" => {
                Ok(Expr::UnOp(Sub1, Box::new(parse_expr(e)?)))
            }
            [Sexp::Atom(S(op)), e] if op == "isnum" => {
                Ok(Expr::UnOp(IsNum, Box::new(parse_expr(e)?)))
            }
            [Sexp::Atom(S(op)), e] if op == "isbool" => {
                Ok(Expr::UnOp(IsBool, Box::new(parse_expr(e)?)))
            }
            ...
            [Sexp::Atom(S(op)), Sexp::List(vec2), e] if op == "let" => {
                Ok(Expr::Let(parse_binds(vec2)?, Box::new(parse_expr(e)?)))
            }
            ...
            [Sexp::Atom(S(op)), Sexp::Atom(S(id)), e2] if op == "set!" => {
                Ok(Expr::Set(id.to_string(), Box::new(parse_expr(e2)?)))
            }
            [Sexp::Atom(S(op)), econd, e1, e2] if op == "if" => Ok(Expr::If(
                Box::new(parse_expr(econd)?),
                Box::new(parse_expr(e1)?),
                Box::new(parse_expr(e2)?),
            )),
            ...
            [Sexp::Atom(S(op)), e] if op == "print" => Ok(Expr::Print(Box::new(parse_expr(e)?))),
...
pub fn parse_fn(s: &Sexp) -> Result<SnekFn, ParseError> {
    match s {
        Sexp::List(vec) => match &vec[..] {
            ...
            {
                match &subvec[..] {
                    [Sexp::Atom(S(fname)), fnargs @ ..] => Ok(SnekFn {
                        name: fname.clone(),
            ...
            [Sexp::Atom(S(fun)), Sexp::List(subvec), e] if fun == "fun" => match &subvec[..] {
                [Sexp::Atom(S(fname)), fnargs @ ..] => Ok(SnekFn {
                    name: fname.clone(),
...
fn typed_args(args: &[Sexp]) -> Result<Vec<(String, Type)>, ParseError> {
    ...
    for arg in args.iter() {
        match arg {
            Sexp::List(vec) => match &vec[..] {
                [Sexp::Atom(S(arg_name)), Sexp::Atom(S(colon)), Sexp::Atom(S(arg_type))]
                    if colon == ":" =>
                {
...
fn parse_bind(binding: &Sexp) -> Result<(String, Expr), ParseError> {
    match binding {
        Sexp::List(vec) => match &vec[..] {
            [Sexp::Atom(S(name)), e] => Ok((name.clone(), parse_expr(e)?)),
